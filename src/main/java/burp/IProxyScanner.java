...
public class IProxyScanner implements IProxyListener, IContextMenuFactory {
    ...
    public void processProxyMessage(boolean messageIsRequest, final IInterceptedProxyMessage iInterceptedProxyMessage) {
        //动态处理弱口令爆破线程池
        if (!FingerTab.functionButton.isSelected() && !WeakPasswordTab.weakPasswordBlasting.isSelected()){
            startupMonitorExecutor();
        } else {
            shutdownMonitorExecutor();
        }

        if (!messageIsRequest && !FingerTab.functionButton.isSelected()) {
    ...
    private void startupMonitorExecutor() {
        if (monitorExecutor == null || monitorExecutor.isShutdown()){
            monitorExecutor = Executors.newSingleThreadScheduledExecutor();
            startDatabaseMonitor();
            BurpExtender.getStdout().println("[+] Start Run Password Blasting MonitorExecutor Success~ ");
        }
    }
    ...
    public static void shutdownMonitorExecutor() {
        ...
        if (monitorExecutor != null && !monitorExecutor.isShutdown()) {
            ...
            BurpExtender.getStdout().println("[-] Stop Run Password Blasting MonitorExecutor Success~ ");
        }
    }
